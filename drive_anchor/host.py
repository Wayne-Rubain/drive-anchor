...
def unmount(path: str) -> bool:
    """Unmount `path`. Returns True if it is not mounted afterwards.

    A failure here is routine rather than exceptional -- most often the path
    simply was not mounted -- so this reports rather than raises.
    """
